...
const loadBalances = async () => {
  try {
    ...
    if (settlements.length === 0) {
      ...
      `).join('');
      document.getElementById('settlementsContainer').innerHTML = settlementsHTML;
    }
...
const addExpense = async () => {
  ...
  if (!description) {
    document.getElementById('expenseError').textContent = 'Description is required';
    document.getElementById('expenseError').style.display = 'block';
    return;
  }
  if (!amount || amount <= 0) {
    document.getElementById('expenseError').textContent = 'Valid amount is required';
    document.getElementById('expenseError').style.display = 'block';
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      document.getElementById('expenseError').textContent = data.message;
      document.getElementById('expenseError').style.display = 'block';
      return;
    }
    ...
    document.getElementById('expenseError').textContent = 'Something went wrong';
    document.getElementById('expenseError').style.display = 'block';
  }
};
